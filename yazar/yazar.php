    ...
    <body class="bg-amber-50 min-h-screen flex flex-col">
        ...
        <div class="w-5/6 mx-auto bg-slate-50 flex-grow flex">
            <main class="p-8 w-full">
                <button class="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-700 mb-8 font-bold"><a href="haberEkle.php">+ Haber Ekle</a></button>
                <form action="yazar.php" method="GET" class="mb-4 flex">
                ...
                $total_records_query = mysqli_query($baglanti, "SELECT COUNT(*) FROM haberler WHERE title LIKE '%$search%' AND author ='" . $_SESSION['kullaniciAdi'] . "'");
                $total_records = mysqli_fetch_array($total_records_query)[0];

                $total_pages = ceil($total_records / $limit);

                $haberler = mysqli_query($baglanti, "SELECT * FROM haberler WHERE title LIKE '%$search%' AND author = '" . $_SESSION['kullaniciAdi'] . "' ORDER BY id DESC LIMIT $limit OFFSET $offset");
                ...
                while ($satir = mysqli_fetch_array($haberler)) {
                    ...
                    echo '<button class="bg-blue-500 text-white px-4 py-2 rounded"><a href="haberDuzenle.php?haberID=' . $satir['id'] . '">Düzenle</a></button>';
